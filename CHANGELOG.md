...
- **`@jaggerxtrm/pi-extensions` serena-pool cwd-race in `registerSerenaPool` (KAN-110-A, PR #306, `fba57f55`).** `registerSerenaPool` used `ctx?.cwd ?? process.cwd()`; the `??` short-circuited on a stale-but-present `ctx.cwd` (parent repo) when `session_start` fired before pi's cwd settled into the linked worktree. Wrong cwd → wrong `hashToPort` → daemon bound to the parent `--project`; the next correct-cwd session spawned a second daemon and orphaned the first. New `resolveSessionCwd` reconciles `ctx.cwd` with `process.cwd()` and trusts `process.cwd()` only when it is itself inside a git work tree (`isInsideGitRepo`), so an unrelated `process.cwd()` (e.g. a test runner in `/tmp`) never wins. The CLI worktree-session launcher already guarantees `process.cwd() === worktreePath` at `session_start`, so this is the canonical reconciliation. New `test6_cwdRace` uses a real `git worktree add` to assert daemon `--project === worktreeRoot` and no orphan state on the parent port; full e2e suite 27/27 green. Complements `@jaggerxtrm/specialists#150` (`supervisor.dispose()` reaps the pi/Serena session) — together the two fixes close the host-memory-exhaustion class observed on the Mercury host 2026-06-22 (3rd recurrence of `mercury-infra/infra-cbl`).
