...
| [Pi Extensions](#pi-extensions) | Current extension source layout is **directory-based** under `packages/pi-extensions/extensions/<name>/index |
...
xt update             # dry-run xtrm-managed asset + global Pi package health for current repo
xt update --apply --root ~/dev  # refresh repos and missing/outdated xt-managed Pi packages
...
xt doctor                   # project + global xt-managed Pi package health
xt pi doctor                # Pi runtime install/settings/package diagnostics
...
- `xt update` refreshes xtrm-managed assets and checks global xt-managed Pi package freshness. Without flags it is a dry-run for the current repo; use `--apply` to write repo asset changes and refresh only missing/outdated managed Pi packages, `--repo <path>` for one target, or `--root <dir>` to discover every repo with `.xtrm/registry.json`. JSON output includes a `packages` block with `missing`, `outdated`, `installed`, `refreshed`, and `failed` package sets.
...
- `xt doctor` checks project-managed Cat B assets plus global xt-managed Pi package health. Text output prints a **Pi packages** section; JSON output includes `piPackages`. Missing/outdated packages return remediation commands (`pi install <package>`). `version-unknown` means npm/latest lookup failed or the operator is offline; doctor reports it but never installs packages.
...
Current extension source layout is **directory-based** under `packages/pi-extensions/extensions/<name>/index.ts`, but runtime delivery is package-based through xt-managed Pi packages:
...
- `docs/hooks.md`
- `docs/policies.md`
- `docs/pi-extensions.md` (if Pi package/runtime behavior changed)
- `docs/xtrm-directory.md` (if `.xtrm`/`.pi` runtime ownership changed)
- `docs/cli-architecture.md` (if command JSON/text contracts changed)
